...
function _sendWithSocket($to, $toName, $subject, $htmlBody, $textBody = '') {
    ...
    // Plain TCP first, then STARTTLS upgrade
    ...
    $authResp = _smtp_cmd($sock, "MAIL FROM:<" . GMAIL_USER . ">");
    if (strpos($authResp, '535') !== false) { fclose($sock); return false; } // wrong credentials
...
function sendAdminCreditNotification(string $adminEmail, array $demande): bool {
    $title = "Nouvelle demande de crédit reçue";
    $montant  = number_format((float)$demande['montant'], 2, ',', ' ');
    $duree    = (int)$demande['duree_mois'];
    $taux     = (float)$demande['taux_interet'];
    $date     = htmlspecialchars($demande['date_demande']);
    $clientId = (int)$demande['client_id'];

    $content = '
        <p>Bonjour <strong>Administrateur</strong>,</p>
        <p>Un client vient de soumettre une nouvelle demande de crédit sur la plateforme LegalFin. Voici le récapitulatif :</p>
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
               style="margin:20px 0; border:1px solid #e5e7eb; border-radius:8px; overflow:hidden;">
            <tr>
                <td style="background-color:#3b82f6; width:5px;"></td>
                <td style="padding:15px 20px; background-color:#f9fafb;">
                    <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                        <tr>
                            <td style="padding-bottom:8px; font-size:14px; color:#6b7280;">Client ID :</td>
                            <td style="padding-bottom:8px; font-size:15px; font-weight:600; color:#1f2937; text-align:right;">' . $clientId . '</td>
                        </tr>
                        <tr>
                            <td style="padding-bottom:8px; font-size:14px; color:#6b7280;">Montant demandé :</td>
                            <td style="padding-bottom:8px; font-size:15px; font-weight:600; color:#1f2937; text-align:right;">' . $montant . ' TND</td>
                        </tr>
                        <tr>
                            <td style="padding-bottom:8px; font-size:14px; color:#6b7280;">Durée :</td>
                            <td style="padding-bottom:8px; font-size:15px; font-weight:600; color:#1f2937; text-align:right;">' . $duree . ' mois</td>
                        </tr>
                        <tr>
                            <td style="padding-bottom:8px; font-size:14px; color:#6b7280;">Taux d\'intérêt :</td>
                            <td style="padding-bottom:8px; font-size:15px; font-weight:600; color:#1f2937; text-align:right;">' . $taux . ' %</td>
                        </tr>
                        <tr>
                            <td style="font-size:14px; color:#6b7280;">Date de demande :</td>
                            <td style="font-size:15px; font-weight:600; color:#1f2937; text-align:right;">' . $date . '</td>
                        </tr>
                    </table>
                </td>
            </tr>
        </table>
        <p>Veuillez vous connecter à l\'interface d\'administration pour traiter cette demande.</p>
        <p>Cordialement,<br><br><strong>Système LegalFin</strong></p>
    ';

    $html = _getBaseEmailLayout($title, $content);
    return sendMail(
        $adminEmail,
        'Administrateur LegalFin',
        '🔔 Nouvelle demande de crédit - LegalFin',
        $html
    );
}
?>