...
if ($userId) {
    ...
    $latestUser = $userModel->findById((int)$userId);
    if ($latestUser) {
...
// Show full banking navigation only when KYC is verified.
// If KYC is pending (en_attente) or rejected (REJETE), only show Mon Profil.
$kycStatus = strtoupper($user['status_kyc'] ?? '');
$showBankingNavigation = Session::isLoggedIn() && $kycStatus === 'VERIFIE';
...
<div class="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <?php if (!$showBankingNavigation): ?>
      <div style="margin:.8rem .5rem 0;padding:.75rem;background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.2);border-radius:10px;font-size:.7rem;color:var(--amber);line-height:1.5;">
        <?php if ($kycStatus === 'REJETE'): ?>
          ⚠️ <strong>KYC rejeté.</strong> Votre vérification d'identité a été refusée. Veuillez déposer à nouveau vos documents depuis votre profil.
        <?php else: ?>
          ⏳ <strong>KYC en attente.</strong> Vos sections bancaires seront débloquées après validation de votre identité.
        <?php endif; ?>
      </div>
    <?php endif; ?>
    ...
    <?php if ($showBankingNavigation && Session::isLoggedIn()): ?>
