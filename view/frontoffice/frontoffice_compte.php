...
if (isset($_GET['id_compte']) && $_GET['id_compte'] !== '') {
    $targetId = (int)$_GET['id_compte'];
    foreach ($comptes as $c) { 
        if ((int)$c->getIdCompte() === $targetId) { 
            $selected = $c; 
            break; 
        } 
    }
...
$allTransactions = [];
try {
    $apiUrl = "https://sheetdb.io/api/v1/6vuiivxwuvdwn/search?id_utilisateur=" . $userId;
    $ch = curl_init($apiUrl);
...
<div class="notif-panel" id="notif-panel">
  ...
  <?php if ($pendingCount>0): ?>
  <div class="np-footer">
    <a href="<?= APP_URL ?>/view/frontoffice/frontoffice_compte.php?tab=attente" onclick="closeNotifPanel()">Voir toutes les demandes →</a>
  </div>
  <?php endif; ?>
</div>
...
<div class="cb-widget" id="cb-widget">
  ...
  <div class="cb-panel" id="cb-panel" style="position:relative;">
    ...
    function togglePrivacy() {
        ...
        const mode = isPrivacyActive ? 'hidden' : 'visible';
        localStorage.setItem('privacy_mode', mode);
        localStorage.setItem('privacy', isPrivacyActive); // legacy support
    ...
    var cbUrl='<?= APP_URL ?>/controller/ChatbotController.php';
    function cbToggle(){
        cbIsOpen=!cbIsOpen;
