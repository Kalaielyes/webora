...
// ── Compte actifs (real count) ──────────────────────────────────────────────
require_once __DIR__ . '/../../controller/CompteController.php';
$_allComptes      = CompteController::findByUtilisateur((int)Session::get('user_id'));
$_activeComptes   = array_filter($_allComptes, fn($c) => $c->getStatut() === 'actif');
$_activeCount     = count($_activeComptes);
$_typeLabels      = array_unique(array_map(fn($c) => ucfirst($c->getTypeCompte()), $_activeComptes));
$_typeString      = $_activeCount > 0 ? implode(' · ', $_typeLabels) : 'Aucun compte actif';
  ...
  <div class="content animate-in">
    ...
    <div class="profile-hero">
      <div class="profile-avatar-wrap">
        <div class="profile-avatar">
          ...
          $realPath = __DIR__ . '/../../' . $selfiePath;
          if (!empty($selfiePath) && file_exists($realPath)): 
          ?>
            <img src="<?= APP_URL ?>/<?= htmlspecialchars(ltrim($selfiePath, '/')) ?>" 
                 alt="Avatar" 
      ...
      <div class="profile-info">
        <div class="profile-name privacy-blur"><?= htmlspecialchars(ucwords($user['nom'].' '.$user['prenom'])) ?></div>
        <div class="profile-cin privacy-blur" style="display:flex; align-items:center; gap:8px;">
          <span>CIN : <?= htmlspecialchars($user['cin']) ?></span>
          <span style="opacity:0.3">•</span>
          <span style="color:var(--blue); font-weight:600;">#NXB-<?= str_pad($user['id'],5,'0',STR_PAD_LEFT) ?></span>
        </div>
    ...
    <div class="stats-row">
      <div class="stat-card">
        ...
        <div class="stat-card-val"><?= $_activeCount ?></div><div class="stat-card-sub"><?= htmlspecialchars($_typeString) ?></div>
      </div>
      <div class="stat-card">
    ...
    <div class="two-col">
      <div>
        ...
        <div class="info-card">
          <div class="info-grid">
            <div class="info-field"><div class="info-label">Nom</div><div class="info-value privacy-blur"><?= htmlspecialchars($user['nom']) ?></div></div>
            <div class="info-field"><div class="info-label">Prénom</div><div class="info-value privacy-blur"><?= htmlspecialchars($user['prenom']) ?></div></div>
            <div class="info-field"><div class="info-label">Date de naissance</div><div class="info-value privacy-blur"><?= date('d/m/Y', strtotime($user['date_naissance'])) ?></div></div>
            <div class="info-field"><div class="info-label">Téléphone</div><div class="info-value privacy-blur"><?= htmlspecialchars($user['numTel']) ?></div></div>
            <div class="info-field info-field-full"><div class="info-label">Email</div><div class="info-value privacy-blur"><?= htmlspecialchars($user['email']) ?></div></div>
            <div class="info-field info-field-full"><div class="info-label">Adresse</div><div class="info-value privacy-blur"><?= htmlspecialchars($user['adresse']) ?></div></div>
            <div class="info-field"><div class="info-label">CIN</div><div class="info-value mono privacy-blur"><?= htmlspecialchars($user['cin']) ?></div></div>
