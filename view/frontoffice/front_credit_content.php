...
$self = APP_URL . '/controller/CreditController.php';
$dashboardUrl = APP_URL . '/view/frontoffice/frontoffice_utilisateur.php?page=credit';

// Initialize variables to avoid Undefined Variable warnings
$success = $success ?? '';
$errors = $errors ?? [];

// If flash exists (passed from dashboard), populate variables
if (isset($flash) && !empty($flash)) {
    if ($flash['type'] === 'success') $success = $flash['message'];
    if ($flash['type'] === 'error') $errors = explode(' | ', $flash['message']);
}
?>
      <div class="page <?= $activeTab === 'nouvelle' ? 'on' : '' ?>" id="page-credits">
        ...
        <!-- SECTION TABS -->
        <div class="tabs-crud">
          <div class="tab-c <?= $activeSubTab === 'demande' ? 'on' : '' ?>" onclick="switchTab('demande',this)">
            📋 Nouvelle demande
          </div>
          <div class="tab-c <?= $activeSubTab === 'garantie' ? 'on' : '' ?>" onclick="switchTab('garantie',this)">
            🔒 Nouvelle garantie
          </div>
          <div class="tab-c <?= $activeSubTab === 'simulateur' ? 'on' : '' ?>" onclick="switchTab('simulateur',this)">
            🧮 Simulateur
          </div>
        </div>
        ...
        ———————————————————————————————————————————————————————————————————————— -->
        <div id="tab-demande" style="display:<?= $activeSubTab === 'demande' ? 'block' : 'none' ?>">

          <!-- FORM: NEW/EDIT DEMANDE -->
          <div class="sc" style="margin-bottom:1.5rem;">
            <div class="sc-hd">
              <div class="sc-title">
                <?= $editDemande ? '📝 Modifier demande #' . (int) $editDemande['id'] : '➕ Nouvelle demande de crédit' ?>
              </div>
            </div>
            <div style="padding:1.2rem;">
              <form method="POST" action="<?= $self ?>" id="form-demande" novalidate>
                ...
                <div style="display:flex;gap:.8rem;margin-top:1rem;">
                  ...
                  <?php if ($editDemande): ?>
                    <a href="<?= $dashboardUrl ?>&view=mes-credits" class="sc-link">✖ Annuler</a>
                  <?php endif; ?>
                </div>
              </form>
            </div>
        ...
        <!-- ————————————————————————————————————————————————————————————————————————
             TAB 2: GARANTIES
        ———————————————————————————————————————————————————————————————————————— -->
        <div id="tab-garantie" style="display:<?= $activeSubTab === 'garantie' ? 'block' : 'none' ?>">

          <!-- FORM: NEW/EDIT GARANTIE -->
          <div class="sc" style="margin-bottom:1.5rem;">
            <div class="sc-hd">
              <div class="sc-title">
                <?= $editGarantie ? '📝 Modifier garantie #' . (int) $editGarantie['id'] : '🔒 Ajouter une garantie' ?>
              </div>
            </div>
            <div style="padding:1.2rem;">
    ...
    <div class="err-msg" id="e-g-doc"></div>
                ...
                <div style="display:flex;gap:.8rem;margin-top:1rem;">
                  ...
                  <?php if ($editGarantie): ?>
                    <a href="<?= $dashboardUrl ?>&view=mes-garanties" class="sc-link">✖ Annuler</a>
                  <?php endif; ?>
                </div>
              </form>
            </div>
        ...
        <!-- ————————————————————————————————————————————————————————————————————————
             TAB 3: SIMULATEUR
        ———————————————————————————————————————————————————————————————————————— -->
        <div id="tab-simulateur" style="display:<?= $activeSubTab === 'simulateur' ? 'block' : 'none' ?>">
          <div class="sc cr-sim">
      ...
      ———————————————————————————————————————————————————————————————————————— -->
      <div class="page <?= $activeTab === 'mes-credits' ? 'on' : '' ?>" id="page-mes-credits">
        <div class="sc">
          <div class="sc-hd">
                    ...
                    <tr>
                      ...
                      <td style="text-align:center;">
                        <div class="td-acts-row" style="justify-content:center;">
                          <a href="<?= $dashboardUrl ?>&view=mes-credits&edit_d=<?= $d['id'] ?>" class="btn-edt">📝</a>
      ...
      ———————————————————————————————————————————————————————————————————————— -->
      <div class="page <?= $activeTab === 'mes-garanties' ? 'on' : '' ?>" id="page-mes-garanties">
        <div class="sc">
          <div class="sc-hd">
                ...
                <tbody>
                  <?php foreach ($garanties as $g): ?>
                    <tr>
                      ...
                      <td style="text-align:center;">
                        <div class="td-acts-row" style="justify-content:center;">
                          <a href="<?= $dashboardUrl ?>&view=mes-garanties&edit_g=<?= $g['id'] ?>" class="btn-edt">📝</a>
                          <form method="POST" action="<?= $self ?>" style="display:inline;" onsubmit="return confirm('Êtes-vous sûr ?')">
      ...
      try {
        ...
        if (badge) {
          if (data.completed) {
            ...
            badge.textContent = '📝 En attente'; badge.className = 'badge b-wait';
          }
        }
