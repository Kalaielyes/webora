...
(function () {
  ...
  window.openChequeModal = function (num, nom, iban, id_chequier) {
    // Mémoriser les paramètres pour les passer après vérification
    window._pendingChequeArgs = { num, nom, iban, id_chequier };
    openVerifyModal();
  };
  ...
  async function vcm_success(adminId) {
    ...
    setTimeout(function () {
      ...
      if (a && _originalOpenChequeModal) {
        _originalOpenChequeModal(a.num, a.nom, a.iban, a.id_chequier);
      }
    }, 1200);
  }
  ...
  async function vcm_init() {
    try {
      // Charger les modèles (une seule fois grâce au cache faceapi)
      vcm_setStatus('📦 Chargement modèles...'); vcm_setProgress(20);
      ...
      vcm_setStatus('📷 Activation caméra...'); vcm_setProgress(85);
      ...
      vcm_setStatus('🔄 Chargement profils...'); vcm_setProgress(92);
      const res  = await fetch('../../controller/FaceAuthController.php?action=getDescriptors');
      const data = await res.json();
      vcm_descriptors = data.map(r => ({
        adminId:    r.admin_id,
        descriptor: new Float32Array(JSON.parse(r.face_descriptor))
      }));

      if (vcm_descriptors.length === 0) {
        vcm_setStatus('❌ Aucun profil enregistré.', false);
        return;
      }

      vcm_setProgress(100);
      vcm_setStatus('✅ Prêt. Regardez la caméra...');
      video.addEventListener('loadeddata', vcm_loop, { once: true });
  ...
  // ── 11. Boucle reconnaissance ────────────────────────────────────
  async function vcm_loop() {
    if (vcm_loopRunning) return;
    vcm_loopRunning = true;
    await _vcm_recognize();
  }

  async function _vcm_recognize() {
    if (vcm_done || vcm_faceAttempts >= MAX_FACE) return;

    document.getElementById('vcm_attempts').textContent =
      `Tentative ${vcm_faceAttempts + 1} / ${MAX_FACE}`;

    await new Promise(r => setTimeout(r, 800));
    if (vcm_done) return;

    const video   = document.getElementById('vcm_video');
    const overlay = document.getElementById('vcm_overlay');
    ...
    try {
      ...
      if (!det) {
        vcm_setStatus('👤 Aucun visage détecté. Positionnez-vous.');
        await new Promise(r => setTimeout(r, 1500));
        return _vcm_recognize();
      }
      ...
      for (const a of vcm_descriptors) {
        const d = faceapi.euclideanDistance(det.descriptor, a.descriptor);
      ...
      if (bestDist <= THRESHOLD) {
        ...
        ctx.strokeRect(box.x, box.y, box.width, box.height);
        vcm_setStatus('✅ Identité confirmée ! Ouverture...', true);
        await vcm_success(best.adminId);
        ...
        if (vcm_faceAttempts >= MAX_FACE) {
          ...
          vcm_setStatus('🚫 Visage non reconnu. Utilisez le PIN.', false);
          await fetch('../../controller/FaceAuthController.php?action=logFail', { method: 'POST' });
        } else {
          vcm_setStatus(`❌ Non reconnu. ${MAX_FACE - vcm_faceAttempts} tentative(s) restante(s).`);
          await new Promise(r => setTimeout(r, 1500));
          return _vcm_recognize();
        }
      }
