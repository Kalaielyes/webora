...
<body>
                ...
                <div class="two-col-layout">
                    <div id="tabDemandes" class="table-card demand-card">
                        <div class="table-toolbar">
                            ...
                            <div class="filters">
                                <button class="filter-btn" onclick="filterByStatus('tous')">Tous</button>
                                <button class="filter-btn active" onclick="filterByStatus('en-attente')">En attente</button>
                            </div>
                        </div>
                        <div class="table-scroll">
                            <table id="requestsTable">
                                ...
                                <tbody id="demandesTableBody">
                                    ...
                                    ?>
                                    <tr class="request-row" onclick="showDetail(this)" style="cursor:pointer;" data-status="<?= strtolower(str_replace([' ', 'é'], ['-', 'e'], $statut)) ?>" data-id="<?= $dem['id_demande'] ?>" data-id-compte="<?= $dem['id_compte'] ?>" data-name="<?= htmlspecialchars($dem['nom et prenom']) ?>" data-iban="<?= htmlspecialchars($dem['iban']) ?>" data-date="<?= date('d/m/Y', strtotime($dem['date_demande'])) ?>" data-type="<?= ucfirst($dem['type_chequier'] ?? 'Standard') ?>" data-nb="<?= $dem['nombre_cheques'] ?>" data-montant="<?= $dem['montant_max_par_cheque'] ?>" data-mode="<?= $dem['mode_reception'] ?>" data-tel="<?= $dem['telephone'] ?>" data-email="<?= $dem['email'] ?>" data-statut="<?= $statut ?>" data-selfie="<?= $dem['selfie_path'] ?? '' ?>" data-cin="<?= htmlspecialchars($dem['user_cin'] ?? '—') ?>">
                                        <td>
                                            <div class="client-cell">
                                                <div class="client-avatar">
                                                    <?php if(!empty($dem['selfie_path'])): ?>
                                                        <img src="../../<?= htmlspecialchars($dem['selfie_path']) ?>" onerror="this.style.display='none'">
                                                    <?php endif; ?>
                                                    <span class="client-avatar-initials"><?= htmlspecialchars(demandInitials($fullName)) ?></span>
                                                </div>
                                                <div>
                    ...
                    <div class="detail-panel" id="detailPanel">
                        ...
                        <div class="dp-header" style="display: flex; align-items: center; gap: 1rem; padding-bottom: 1.2rem; border-bottom: 1px solid var(--border);">
                            <div class="dp-av" id="dpAvatarContainer" style="width: 48px; height: 48px; border-radius: 50%; background: linear-gradient(135deg, #2563EB, #0D9488); display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 1.1rem; position:relative; overflow:hidden;">
                                <img id="dpAvatarImg" src="" style="width:100%; height:100%; object-fit:cover; position:absolute; top:0; left:0; z-index:2; display:none;">
                                <span id="dpInitials" style="position:relative; z-index:1;">?</span>
                            </div>
                            <div style="flex: 1;">
    ...
    <script>
        ...
        function showDetail(data) {
            ...
            const dpImg = document.getElementById('dpAvatarImg');
            if(data.selfie) {
                dpImg.src = '../../' + data.selfie;
                dpImg.style.display = 'block';
            } else {
                dpImg.style.display = 'none';
            }
            
            document.getElementById('dpCin').textContent = 'CIN: ' + (data.cin || '—');
        ...
        function openPremiumModal(d) {
            ...
            document.getElementById('hidden_id_compte_modal').value = d.id_compte || d.idCompte || (chq ? (chq.id_compte || '') : '');
