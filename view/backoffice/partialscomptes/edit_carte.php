  ...
  <div class="section-head" style="margin-bottom:.5rem">
    ...
    <a href="<?= APP_URL ?>/view/backoffice/backoffice_compte.php?tab=comptes&id_compte=<?= $carteEdit->getIdCompte() ?>" class="btn-ghost">← Retour</a>
  </div>
  <div style="display:flex; justify-content:center; align-items:flex-start; padding-top:1rem;">
    <div class="edit-card-panel" style="background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:2rem; width:100%; max-width:650px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.1), 0 8px 10px -6px rgba(0,0,0,0.1);">
      ...
      <form method="POST" action="<?= APP_URL ?>/controller/CarteController.php">
        ...
        <div style="margin-top:2.5rem; padding-top:1.5rem; border-top:1px solid var(--border); display:flex; justify-content:flex-end; gap:1rem;">
          <a href="<?= APP_URL ?>/view/backoffice/backoffice_compte.php?tab=comptes&id_compte=<?= $carteEdit->getIdCompte() ?>" class="btn-cancel" style="padding:.75rem 1.5rem; border-radius:10px; font-weight:600; text-decoration:none; color:var(--muted2); background:var(--surface2); display:flex; align-items:center; gap:.5rem;">
            Annuler
          </a>
