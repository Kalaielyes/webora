...
if ($currentUserId_sidebar) {
    $sidebarUser = $m_sidebar->findById($currentUserId_sidebar);
} else {
    $sidebarUser = null;
}
$adminInitials_sidebar = strtoupper(mb_substr($sidebarUser['nom']??Session::get("user_nom")??"",0,1).mb_substr($sidebarUser['prenom']??Session::get("user_prenom")??"",0,1));
...
?>
<style>
/* Sidebar Dropdown & Avatar Styles */
.sb-av { position: relative; overflow: hidden; display: flex; align-items: center; justify-content: center; }
.sb-av img { width: 100%; height: 100%; object-fit: cover; position: absolute; top: 0; left: 0; z-index: 2; }
.sb-av-initials { position: relative; z-index: 1; }
...
<div class="sidebar">
  ...
  <div class="sb-admin">
    <div class="sb-av">
        <?php if (!empty($sidebarUser['selfie_path']) && file_exists(__DIR__ . '/../../../' . $sidebarUser['selfie_path'])): ?>
            <img src="<?= APP_URL ?>/<?= htmlspecialchars($sidebarUser['selfie_path']) ?>" onerror="this.style.display='none'">
        <?php endif; ?>
        <span class="sb-av-initials"><?= $adminInitials_sidebar ?></span>
    </div>
    <div>
      <div class="sb-aname"><?= htmlspecialchars(($sidebarUser['nom']??Session::get("user_nom")??"")." ".($sidebarUser['prenom']??Session::get("user_prenom")??"")) ?></div>
  ...
  <nav class="sb-nav">
    ...
    <!-- User Dropdown -->
    <div class="nav-dropdown <?= ($currentFile_sidebar === "backoffice_utilisateur.php" && ($page_sidebar === "utilisateurs" || $page_sidebar === "statistiques")) ? "open" : "" ?>" id="dropdown-user-admin">
      <div class="nav-item nav-dropdown-toggle <?= (!in_array("utilisateurs",$myModules_sidebar) && !in_array("statistiques",$myModules_sidebar))?"disabled":"" ?>" 
           onclick="toggleDropdown('dropdown-user-admin')"
      ...
      <div class="nav-dropdown-menu">
        <a class="nav-sub-item <?= ($currentFile_sidebar === "backoffice_utilisateur.php" && $page_sidebar==="utilisateurs")?"active":"" ?>" href="<?= APP_URL ?>/view/backoffice/backoffice_utilisateur.php?page=utilisateurs&filtre=tous">
          <span class="nav-sub-dot"></span>Compte
          <?php if($kyc_count>0): ?><span class="nav-badge" style="margin-left:auto"><?= $kyc_count ?></span><?php endif; ?>
        </a>
        <a class="nav-sub-item <?= ($currentFile_sidebar === "backoffice_utilisateur.php" && $page_sidebar==="statistiques")?"active":"" ?>" href="<?= APP_URL ?>/view/backoffice/backoffice_utilisateur.php?page=statistiques">
    ...
    <div class="nav-dropdown <?= ($currentFile_sidebar === "backoffice_compte.php" || $page_sidebar === "comptes") ? "open" : "" ?>" id="dropdown-compte-admin">
      <div class="nav-item nav-dropdown-toggle <?= !in_array("comptes",$myModules_sidebar)?"disabled":"" ?>" 
        ...
        <span style="flex:1">Compte & Carte</span>
        <svg class="nav-chevron" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="margin-left:auto"><path d="M6 9l6 6 6-6"/></svg>
      </div>
      <div class="nav-dropdown-menu">
