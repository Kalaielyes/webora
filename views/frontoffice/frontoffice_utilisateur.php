...
$selectedCompte = !empty($comptes) ? $comptes[0] : null;
?>
<!DOCTYPE html>
<html lang="fr">
<head>
  ...
  <div class="content">
    ...
    <div class="profile-hero">
      ...
      <div class="profile-info">
        ...
        <div class="profile-cin">CIN : <?= htmlspecialchars($user['cin'] ?? 'Non renseigné') ?> &nbsp;·&nbsp; #NXB-<?= str_pad($user['id'] ?? 0, 5, '0', STR_PAD_LEFT) ?></div>
        <div class="profile-badges">
          <span class="pbadge pbadge-kyc"><svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg> KYC <?= $user['status_kyc'] ?? 'INCONNU' ?></span>
          <span class="pbadge pbadge-aml"><svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg> AML <?= $user['status_aml'] ?? 'INCONNU' ?></span>
          <span class="pbadge pbadge-actif"><span class="dot-pulse" style="background:var(--teal)"></span> <?= $user['status'] ?? 'ACTIF' ?></span>
        ...
        <div class="profile-joined">Membre depuis le <?= !empty($user['date_inscription']) ? date('d/m/Y', strtotime($user['date_inscription'])) : 'Inconnu' ?></div>
      </div>
    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        ...
        <div class="stat-card-val" style="font-size:1.1rem"><?= !empty($user['derniere_connexion']) ? date('d/m H:i', strtotime($user['derniere_connexion'])) : 'Maintenant' ?></div>
