...
<div class="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <a href="<?= APP_URL ?>/views/backoffice/backoffice_utilisateur.php?page=dashboard" class="nav-item <?= ($currentFile === 'backoffice_utilisateur.php' && $page === 'dashboard') ? 'active' : '' ?>">
    ...
    <div class="nav-dropdown <?= $currentFile === 'backoffice_compte.php' ? 'open' : '' ?>" id="dropdown-compte-admin">
      ...
      <div class="nav-dropdown-menu">
        <a class="nav-sub-item <?= ($currentFile === 'backoffice_compte.php' && $tab==='comptes')?'active':'' ?>" href="<?= APP_URL ?>/views/backoffice/backoffice_compte.php?tab=comptes">
          <span class="nav-sub-dot"></span>Liste des comptes
        </a>
        <a class="nav-sub-item <?= ($currentFile === 'backoffice_compte.php' && $tab==='attente')?'active':'' ?>" href="<?= APP_URL ?>/views/backoffice/backoffice_compte.php?tab=attente">
        ...
        <a class="nav-sub-item <?= ($currentFile === 'backoffice_compte.php' && $tab==='stats')?'active':'' ?>" href="<?= APP_URL ?>/views/backoffice/backoffice_compte.php?tab=stats">
          <span class="nav-sub-dot"></span>Statistiques
        </a>
      </div>
    </div>

    <a href="<?= APP_URL ?>/views/backoffice/backoffice_utilisateur.php?page=utilisateurs" class="nav-item <?= ($currentFile === 'backoffice_utilisateur.php' && $page === 'utilisateurs') ? 'active' : '' ?>">
    ...
    <a href="<?= APP_URL ?>/views/backoffice/backoffice_utilisateur.php?page=audit" class="nav-item <?= ($currentFile === 'backoffice_utilisateur.php' && $page === 'audit') ? 'active' : '' ?>">
    ...
    <a href="<?= APP_URL ?>/views/backoffice/backoffice_utilisateur.php?page=permissions" class="nav-item <?= ($currentFile === 'backoffice_utilisateur.php' && $page === 'permissions') ? 'active' : '' ?>">
    ...
    <a href="<?= APP_URL ?>/views/frontoffice/frontoffice_compte.php" class="nav-item" style="color: var(--blue);">
      <svg fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
      Retour Frontoffice
    </a>

    <a href="<?= APP_URL ?>/controllers/AuthController.php?action=logout" class="nav-item" style="margin-top:auto; color:var(--rose);">
