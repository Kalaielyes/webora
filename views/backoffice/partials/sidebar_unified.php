...
<div class="sidebar">
  ...
  <div class="sb-admin">
    ...
    ?>
    <?php if(!empty($sidebarUser['selfie_path'])): ?>
      <div class="sb-av" style="padding:0;overflow:hidden"><img src="../../<?= htmlspecialchars($sidebarUser['selfie_path']) ?>" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:inherit"></div>
    <?php else: ?>
  ...
  <nav class="sb-nav">
    ...
    <div class="nav-dropdown <?= ($currentFile_sidebar==="backoffice_utilisateur.php" && in_array($page_sidebar, ['utilisateurs','statistiques'])) ? 'open' : '' ?>" id="dropdown-utilisateur-admin">
      <div class="nav-item nav-dropdown-toggle"
           onclick="toggleDropdown('dropdown-utilisateur-admin')"
      ...
      <div class="nav-dropdown-menu">
        <a class="nav-sub-item <?= ($currentFile_sidebar==="backoffice_utilisateur.php" && $page_sidebar==="utilisateurs")?"active":"" ?> <?= !in_array("utilisateurs",$myModules_sidebar)?"disabled":"" ?>" href="backoffice_utilisateur.php?page=utilisateurs&filtre=tous">
          <span class="nav-sub-dot"></span>Utilisateurs
          <?php if($kyc_count>0): ?><span class="nav-badge"><?= $kyc_count ?></span><?php endif; ?>
        </a>
        <a class="nav-sub-item <?= ($currentFile_sidebar==="backoffice_utilisateur.php" && $page_sidebar==="statistiques")?"active":"" ?> <?= !in_array("statistiques",$myModules_sidebar)?"disabled":"" ?>" href="backoffice_utilisateur.php?page=statistiques">
