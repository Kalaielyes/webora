...
function generateAttestation(idChequier) {
    ...
    window.open(`${baseUrl}/view/frontoffice/attestation_pdf.php?id=${idChequier}`, '_blank');
}

function generateChequeAttestation(idCheque) {
    const baseUrl = window.APP_URL || '../..';
    window.open(`${baseUrl}/view/frontoffice/attestation_pdf.php?id_cheque=${idCheque}`, '_blank');
}
