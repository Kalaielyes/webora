...
class Score
{
    ...
    public static function recalculateForUser(int $userId): void
    {
        ...
        if ($existing) {
            ...
            $ins = $pdo->prepare("
                INSERT INTO score (user_id, trust_score, score_details, updated_at)
                VALUES (:user_id, :trust_score, :score_details, CURRENT_TIMESTAMP)
            ");
            $ins->execute($payload);
        }
    }
    ...
    {
        $sql = "SELECT s.user_id, s.trust_score, s.score_details, s.updated_at,
