...
class CompteController
{
    ...
    public static function delete(int $id): void
    {
        ...
        if ($iban) {
            $base = 'https://sheetdb.io/api/v1/6vuiivxwuvdwn';
    ...
    {
        $apiUrl = "https://sheetdb.io/api/v1/6vuiivxwuvdwn";
            ...
            case 'virement': {
                ...
                if ($destType === 'interne') {
                    ...
                    if ($obj && $obj->getIdUtilisateur() === $idUser) {
                        // Convert user-entered amount (in $deviseInput) to objective's currency
                        $rateToObj = $rates[$deviseInput][$obj->getDevise()] ?? 1.0;
                        $montantTarget = $montant * $rateToObj;

                        $res = ObjectifController::alimenter($idUser, $idObj, $idSource, $montantTarget); 
                        if (!$res['success']) {
