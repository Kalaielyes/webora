...
require_once __DIR__ . '/DonAIService.php';
...
try {
    ...
    // Fallback if AI failed or returned unknown campaign_id
    if ($selected === null) {
        usort($campaigns, static function (array $a, array $b): int {
            $aDays    = max(0, (int)$a['days_left']);
            $bDays    = max(0, (int)$b['days_left']);
            $aUrgency = ((float)$a['remaining_amount'] * 0.65)
                        + ((100 - (float)$a['percentage_funded']) * 30)
                        + ($aDays <= 7 ? 40 : 0);
            $bUrgency = ((float)$b['remaining_amount'] * 0.65)
                        + ((100 - (float)$b['percentage_funded']) * 30)
                        + ($bDays <= 7 ? 40 : 0);
            if ($aUrgency === $bUrgency) return $aDays <=> $bDays;
            return $bUrgency <=> $aUrgency;
        });

        $selected     = $campaigns[0];
        $urgencyScore = max(1, min(100, (int)round(
            (100 - (float)$selected['percentage_funded']) + (max(0, 15 - (int)$selected['days_left']) * 2)
        )));
        $explanation  = 'Sélectionné par le classement d\'urgence de secours : montant restant important, taux de financement faible et temps limité.';
    }

    // Trim explanation to 120 words max
