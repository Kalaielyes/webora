...
Session::start(); // Must be called before reading $_SESSION
    ...
    if ($action === 'submit_investment' || $action === 'update_investment') {
        ...
        try {
            if ($action === 'submit_investment') {
                $idCompte = (int)($_POST['financement_source'] ?? 0);
                if ($idCompte <= 0) {
                    echo json_encode(['success' => false, 'message' => 'Veuillez sélectionner un compte de financement.']);
                    exit;
                }

                $compte = CompteController::findById($idCompte);
                if (!$compte || $compte->getIdUtilisateur() !== $userId || $compte->getStatut() !== 'actif') {
                    echo json_encode(['success' => false, 'message' => 'Compte de financement invalide ou inactif.']);
                    exit;
                }

                if ($compte->getSolde() < $amount) {
                    echo json_encode(['success' => false, 'message' => 'Solde insuffisant sur le compte sélectionné.']);
                    exit;
                }
                ...
                CompteController::update($compte);

                // Fetch project title for the log
                $project = Projet::getProjectById((int)$projectId);
                $projectTitle = $project ? $project['titre'] : ('Projet #' . $projectId);

                // Log to SheetDB
                CompteController::logToSheetDB([
                    'id_utilisateur' => $userId,
                    'date'           => date('Y-m-d H:i:s'),
                    'montant'        => $amount,
                    'devise'         => $compte->getDevise(),
                    'type'           => 'investissement',
                    'libelle'        => "Investissement : " . $projectTitle,
                    'source'         => $compte->getIban(),
                    'destination'    => "PROJET_ID_" . $projectId
                ]);

                recalculateScoreForInvestmentStakeholders($investmentId, (int)$projectId, $userId);
                echo json_encode(['success' => true, 'message' => 'Investissement ajouté et compte débité.', 'investment_id' => $investmentId]);
                exit;
            }
