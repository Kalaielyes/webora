    ...
    public static function handleRequest(): void
    {
        ...
        switch ($action) {
            case 'add': {
                ...
                if ($kyc !== 'VERIFIE') {
                    header('Location: ' . APP_URL . '/view/frontoffice/frontoffice_compte.php' . ($idCompte ? '?id_compte='.$idCompte.'&error=kyc_required' : '?error=kyc_required'));
                    exit;
                }
                ...
                if (!empty($errors)) {
                    ...
                    header('Location: ' . APP_URL . '/view/frontoffice/frontoffice_compte.php?form=carte' . ($idCompte ? '&id_compte='.$idCompte : ''));
                    exit;
                }

                self::demanderCarte($idCompte, $type, $titulaire, $reseau, $dateExp, $cvv, $plafondPay, $plafondRet, '');
                header('Location: ' . APP_URL . '/view/frontoffice/frontoffice_compte.php?id_compte=' . $idCompte . '&ok=carte_demande');
                exit;
            }
            ...
            case 'update': {
                ...
                if ($carte) {
                    ...
                    if (!empty($errors)) {
                        $_SESSION['form_errors'] = $errors;
                        $_SESSION['form_data'] = $_POST;
                        header('Location: ' . APP_URL . '/view/backoffice/backoffice_compte.php?tab=comptes&id_compte=' . $idCompteRedir . '&edit_carte=' . $idCarte);
                        exit;
                    }
        ...
        if ($origin === 'front' && $idCompteRedir) {
            header('Location: ' . APP_URL . '/view/frontoffice/frontoffice_compte.php?id_compte=' . $idCompteRedir);
        } elseif ($idCompteRedir) {
            header('Location: ' . APP_URL . '/view/backoffice/backoffice_compte.php?tab=comptes&id_compte=' . $idCompteRedir);
        } else {
